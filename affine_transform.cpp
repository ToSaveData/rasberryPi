...
uchar bilinear_value(Mat img, int x, int y)      // 단일 화소 양선형 보간
{
    if (x >= img.cols - 1) x--;
    if (y >= img.rows - 1) y--;
...
int main()
{
   ...
   warpAffine(image, dst3, rot_map, image.size(), INTER_LINEAR);
